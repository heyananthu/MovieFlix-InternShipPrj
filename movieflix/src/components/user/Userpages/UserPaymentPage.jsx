...
function UserPaymentPage() {
    ...
    const [subprice, setPrice] = useState({})
    useEffect(() => {
        axios.get(`http://localhost:5000/getsubscription/${subscriptionId}`)
            .then((res) => {
    ...
    const payHandler = async (e) => {
        e.preventDefault();
        try {
            // Validate and send card details
            const paymentResponse = await axios.post("http://localhost:5000/carddetails", { name, cardnumber, expiry, cvv });
            if (paymentResponse.status === 200) {
                toast.success('Payment Successful', { position: "top-center", autoClose: 5000 });

                // Update user subscription details
                const subscriptionResponse = await axios.put(`http://localhost:5000/addusersubscription/${userid}/${subscriptionId}`, {
                    isSubscribed: true,
                });
                if (subscriptionResponse.status === 200) {
                    toast.success('Subscription Activated!', { position: "top-center", autoClose: 5000 });
                } else {
                    toast.error('Subscription Update Failed. Please contact support.');
                }
            } else {
    ...
    return (
        <div className='dark:bg-gray-900 h-screen'>
            <div className=''>
                <section class="bg-white py-8 antialiased dark:bg-gray-900 h-screen md:py-16 ">
                        ...
                        <div class="mx-auto max-w-5xl">
                            ...
                            <div class="mt-6 sm:mt-8 lg:flex lg:items-start lg:gap-12">
                                <form action="#" class="w-full rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800 sm:p-6 lg:max-w-xl lg:p-8">
                                    ...
                                    <button type="submit" class="flex w-full items-center justify-center rounded-lg bg-green-700 px-5 py-2.5 text-sm font-medium text-white hover:bg-green-800 focus:outline-none focus:ring-4  focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800" onClick={payHandler}>Pay now</button>
                                </form>
                                    ...
                                    <div class="space-y-4 rounded-lg border border-gray-100 bg-gray-50 p-6 dark:border-gray-700 dark:bg-gray-800">
                                        <div class="space-y-2">
                                            <dl class="flex items-center justify-between gap-4">
                                                ...
                                                <dd class="text-base font-medium text-gray-900 dark:text-white">{subprice.price || 'Loading...'}</dd>
                                            </dl>
                                        ...
                                        <dl class="flex items-center justify-between gap-4 border-t border-gray-200 pt-2 dark:border-gray-700">
                                            ...
                                            <dd class="text-base font-bold text-gray-900 dark:text-white">{subprice.price || 'Loading...'}</dd>
                                        </dl>
                                    </div>
